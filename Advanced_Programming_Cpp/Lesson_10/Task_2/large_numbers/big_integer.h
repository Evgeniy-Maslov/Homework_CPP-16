...
public:
    big_integer(std::string&& x = 0) : number_(new std::vector<int>) {
        for (int i = x.length(); i > 0; i -= a_digit_number) {
    ...
    big_integer(const int& x) : number_(new std::vector<int>(x)) { }
